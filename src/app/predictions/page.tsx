  ...
  return (
    <main className="min-h-screen px-4 py-8 sm:py-12">
      <div className="mx-auto w-full max-w-4xl">
        ...
        <section className="mb-8 rounded-3xl border border-surface-border bg-surface p-6 shadow-2xl shadow-gold/5 sm:p-8">
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.35em] text-gold">World Cup 2026</p>
          <h1 className="text-3xl font-bold sm:text-5xl">Pick the <span className="gold-text-gradient">result</span></h1>
              ...
              >
                <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    ...
                    <h2 className="mt-2 text-xl font-bold text-white sm:text-2xl">{match.home_team} <span className="text-gold">vs</span> {match.away_team}</h2>
                  </div>
                  <span className={`w-fit rounded-full px-3 py-1 text-xs font-semibold ${locked ? "bg-red-500/10 text-red-300" : "bg-gold/10 text-gold"}`}>
                    {locked ? "Locked" : "Open"}
                  </span>
                </div>
                    ...
                    <button
                      ...
                      className={`rounded-2xl border px-3 py-3 text-sm font-bold transition disabled:cursor-not-allowed disabled:opacity-50 sm:px-5 sm:py-4 ${
                        selected === pick
                          ...
                          : "border-surface-border bg-background text-gray-200 hover:border-gold/60 hover:text-gold"
                      }`}
                    >
                      {pickLabels[pick]}
                    </button>
