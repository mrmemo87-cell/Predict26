...
const ledgerActionMessages: Record<string, string> = {
  ...
  dry_run_backfill: "Dry run complete. This did not write ledger rows or affect user points.",
  backfilled: "Backfill complete. Ledger rows may have been inserted, but user points and leaderboards were not changed.",
};
...
const parseMismatchUsers = (value: unknown): MismatchUser[] => {
  ...
  return value.filter((item): item is MismatchUser => Boolean(item && typeof item === "object"));
};
      ...
      <div className="mx-auto w-full max-w-6xl">
        ...
        <section className="mt-8 rounded-3xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.2em] text-gold">Scoring Ledger Status</p>
              <h2 className="mt-2 text-2xl font-bold text-gray-900">Exact-result shadow ledger</h2>
              <p className="mt-2 max-w-3xl text-sm leading-6 text-gray-500">
                Admin-only verification for the exact-score/result ledger shadow. This panel compares already-scored predictions, active ledger rows, and current profile totals. This does not affect user points.
              </p>
            </div>
            <span
          ...
          {(ledgerActionMessage || ledgerErrorMessage) && (
            ...
            >
              <p className="font-bold">{ledgerErrorMessage ? "Action failed" : "Action complete"}</p>
              <p>{ledgerErrorMessage ?? ledgerActionMessage}</p>
              {params.ledger_rows_to_insert && (
                <p>Dry-run rows to insert: {params.ledger_rows_to_insert} · points: {params.ledger_points_to_insert ?? "0"}</p>
              )}
              {params.ledger_inserted_rows && (
                <p>Inserted rows: {params.ledger_inserted_rows} · points recorded: {params.ledger_inserted_points ?? "0"}</p>
              )}
            </div>
          )}
          ...
          {mismatchUsers.length > 0 && (
            <div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900">
              ...
              <p className="mt-1">Showing up to 50 users returned by the service-role reconciliation helper.</p>
                ...
                <table className="min-w-full text-left text-xs">
                  ...
                  <tbody>
                    {mismatchUsers.map((user) => (
                      <tr key={user.user_id ?? `${user.predictions_points}-${user.ledger_points}-${user.profile_points}`}>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <form action={runExactResultLedgerReconciliation}>
              <button type="submit" className="rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-bold text-gray-700 transition hover:border-gold hover:text-gold">
                Run reconciliation
              </button>
            </form>
            <form action={dryRunExactResultLedgerBackfill}>
              <button type="submit" className="rounded-full border border-amber-300 bg-amber-50 px-4 py-2 text-sm font-bold text-amber-800 transition hover:border-amber-400 hover:bg-amber-100">
                Dry-run exact-result ledger backfill
              </button>
            </form>
            <form action={runExactResultLedgerBackfill}>
              <button type="submit" className="rounded-full bg-emerald-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-emerald-700">
